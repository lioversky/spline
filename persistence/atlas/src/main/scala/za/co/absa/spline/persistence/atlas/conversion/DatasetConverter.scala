...
object DatasetConverter {
   ...
   */
  def convert(lineage: DataLineage, dataTypeIdMap: Map[UUID, Id], attributeIdMap: Map[UUID, Id], clusterName: String): Seq[Dataset] = {
    for {
      ...
      operation match {
        ...
        case op.HiveRelation(_, st, table) => {
          ...
          new EndpointDataset(name, qualifiedName, attributes, tableEntity, EndpointType.hive_table, EndpointDirection.input, st)
        }
        case op.Write(_, dt, path, _, _, _) =>
          new EndpointDataset(name, qualifiedName, attributes, new FileEndpoint(path, path), EndpointType.file, EndpointDirection.output, dt)
        case InsertIntoTable(_, dt, path, _, _, _, table) =>
          dt match {
            case "table" =>
              val tableEntity = getTableEntity(table, clusterName)
              new EndpointDataset(name, qualifiedName, attributes, tableEntity, EndpointType.hive_table, EndpointDirection.output, dt)
            case _ =>
              new EndpointDataset(name, qualifiedName, attributes, new FileEndpoint(path, path), EndpointType.file, EndpointDirection.output, dt)
          }
  ...
  def getTableEntity(table: za.co.absa.spline.model.HiveTable, clusterName: String): HiveTable = {
    ...
    val tableEntity = new HiveTable(table.id, table.name, table.db.name, table.owner, table.comment, table.tableType, clusterName, dbEntity)
